...
export default function GrupaPDFGenerator({ grupa, smjer, polaznici }) {
    ...
    const generirajPDF = async () => {
        ...
        doc.text('EVIDENCIJA SMJEROVA, GRUPA I POLAZNIKA', 20, 27);

        // Naslov dokumenta
        doc.setFont('Roboto', 'bold');
        doc.setFontSize(16);
        doc.setTextColor(0, 0, 0);
        doc.text('POPIS GRUPA', 20, 45);
        ...
        doc.text('Podaci o grupi:', 20, yPosition);
        yPosition += 10;

        doc.setFontSize(11);
        doc.setFont(undefined, 'normal');
        doc.text(`Naziv: ${grupa.naziv}`, 25, yPosition);
        yPosition += 7;
        doc.text(`Broj polaznika: ${grupa.polaznici ? grupa.polaznici.length : 0}`, 25, yPosition);
        yPosition += 15;

        // Podaci o smjeru
        doc.setFontSize(14);
        doc.setFont(undefined, 'bold');
        doc.text('Podaci o smjeru:', 20, yPosition);
        yPosition += 10;

        doc.setFontSize(11);
        doc.setFont(undefined, 'normal');
        doc.text(`Naziv: ${smjer.naziv}`, 25, yPosition);
        yPosition += 7;
        doc.text(`Trajanje: ${smjer.trajanje} sati`, 25, yPosition);
        yPosition += 7;
        doc.text(`Cijena: ${smjer.cijena} EUR`, 25, yPosition);
        yPosition += 7;
        doc.text(`Datum pokretanja: ${new Date(smjer.datumPokretanja).toLocaleDateString('hr-HR')}`, 25, yPosition);
        yPosition += 7;
        doc.text(`Aktivan: ${smjer.aktivan ? 'Da' : 'Ne'}`, 25, yPosition);
        yPosition += 15;

        // Popis polaznika
        doc.setFontSize(14);
        doc.setFont(undefined, 'bold');
        doc.text('Popis polaznika:', 20, yPosition);
        yPosition += 10;

        if (polaznici && polaznici.length > 0) {
            // Tablica s polaznicima
            const tableData = polaznici.map(polaznik => [
                polaznik.ime,
                polaznik.prezime,
                polaznik.email,
                polaznik.oib
