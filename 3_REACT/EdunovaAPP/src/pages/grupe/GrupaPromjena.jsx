...
export default function GrupaPromjena() {
    ...
    useEffect(() => {
        if (grupa.polaznici && polaznici.length > 0) {
            const odabrani = polaznici.filter(p => grupa.polaznici.includes(p.sifra))
            setOdabraniPolaznici(odabrani)
        }
    }, [grupa, polaznici])

    async function ucitajGrupu() {
        await GrupaService.getBySifra(params.sifra).then((odgovor) => {
            if (!odgovor.success) {
                alert('Nije implementiran servis')
                return
            }
            setGrupa(odgovor.data)
        })
    }
    ...
    async function promjeni(grupa) {
        await GrupaService.promjeni(params.sifra, grupa).then(() => {
                                ...
                                <Card.Body>
                                    ...
                                    <Form.Group controlId="smjer" className="mb-3">
                                        ...
                                        <Form.Select
                                            ...
                                            value={grupa.smjer || ''}
                                            onChange={(e) => setGrupa({ ...grupa, smjer: parseInt(e.target.value) })}
                                        >
